...
public class CheckTaskToFinish extends HttpSecurityRequest {

    private Context context;

    public CheckTaskToFinish(Context context) {
    ...
    public void processRequest(AsyncHttpServerRequest request, AsyncHttpServerResponse response) {
        ...
        try {
            ...
            finished = workHelper.canStart(taskId);
            Gson gson = GSONBuilder.createGSON();
            Type type = new TypeToken<Boolean>() {
            }.getType();
            response.send(gson.toJson(finished, type));
