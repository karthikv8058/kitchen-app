...
export default class StationList extends AbstractComponent<Props, State> {
  ...
  loadToolbarItems = () => {
    this.tooblarItems.push(
      <TouchableOpacity
        activeOpacity={0.8}
        onPress={this.setUserStations.bind(this)}
          ...
          {t("station-list.hi") +
            ...
            `${
              this.state.stationList.length > 0
                ? t("station-list.work-station")
                : t("station-list.no-station")
            }`}
        </Text>
        <FlatList
  ...
  returnItems(stations: any, index: number) {
    let likelyHood = 0;
    let itemColor = stations.changeUser
      ? "#fffff"
      : convertRGBToHex(getFontColorTone(likelyHood, stations.color));
    let isChangeUser: boolean = stations.changeUser ? true : false;
    return (
      <View style={{ flexDirection: "row", flex: 1 }}>
        <TouchableOpacity
          style={{ flex: 0.9 }}
          onPress={() => this.selectStation(index, stations.uuid, isChangeUser)}
        >
          <View
            style={{
              flexDirection: "row",
          ...
          >
            <CheckBox
              ...
              onClick={() =>
                this.selectStation(index, stations.uuid, isChangeUser)
              }
            />
            {stations.changeUser ? (
              <Text
                onPress={this.expireLogout}
              ...
              <Text
                style={this.checkStyle(
                  this.state.isStationSelected[index],
                  itemColor
                )}
              >
                {stations.name}
        ...
        {!!this.props.route.params.fromTasks && stations.printer_uuid ? (
          <TouchableOpacity
            onPress={() => this.testStationPrinter(stations.uuid)}
            style={{ flex: 0.1, marginTop: 20, marginLeft: 10 }}
          >
        ...
        {!!this.props.route.params.fromTasks && stations.printer_uuid ? (
          <TouchableOpacity
            onPress={() => this.generateStationQRcode(stations.uuid)}
            style={{ flex: 0.1, marginTop: 20, marginLeft: 10 }}
          >
  ...
  generateStationQRcode = (stationId: any) => {
    this.userService.generateStationQRcode(stationId).then((response) => {
      if (!!response) {
        ToastAndroid.show("QR generated successfully", ToastAndroid.SHORT);
      } else {
  ...
  renderStation(stations: any, index: number) {
    let isChangeUser: boolean = stations.changeUser ? true : false;
    return isChangeUser
      ? this.returnChnageUser(stations, index)
      : this.returnItems(stations, index);
  }
