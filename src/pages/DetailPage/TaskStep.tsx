...
class TaskStep extends AbstractComponent<Props, State> {
    ...
    renderImage(fileName: any) {
        ...
        return <FullWidthImage
            ...
            index={this.props.index} item={fileName}
    ...
    renderQuantityAndUnit(data: IIngredientWithQuantity) {
        ...
        let value = (this.props.work && data.recipe.outputQuantity) ? data.quantity : data.quantity;
        let outputValue = symbol != undefined ? "(" + value + " " + symbol + ")" : "(" + value + ")"
    ...
    renderTaskQuantityAndUnit(data: ITaskWithQuantity) {
        ...
        let value = (data.task.outputQuantity) ? data.task.outputQuantity * data.quantity : data.quantity;
        let outputValue = symbol != undefined ? "(" + value + " " + symbol + ")" : "(" + value + ")"
        return outputValue
    }

    renderIngredientImage = (image: string | undefined) => {
        let img = image ? { uri: this.apiBuilder.paths!.imageUrl + image } : placeHolder;
        return (
            <Image style={{
                height: 50,
                width: 50,
    ...
    renderIngredients() {
        let hasIngredients = this.props.step.ingredients && this.props.step.ingredients.length;
        ...
        return (
            <>
                {hasTasks ? (<>
                    ...
                    {this.props.step.tasks.map((data: ITaskWithQuantity, key: number) => {
                        return (
                ...
                {hasIngredients ? (<>
                    ...
                    {this.props.step.ingredients.map((data: IngredientWithQuantity, key: number) => {
                        return (
                            <View key={key} style={styles.ingredientContainer}>
                                {this.renderIngredientImage(data.image)}
                                <Text style={styles.ingredientText}>{data.name} ({data.qty})</Text>
                            </View>
                        );
                    })}
                </>) : null}
            ...
            <ScrollView onLayout={this.onLayout.bind(this)}
                ...
                <View style={{
                    ...
                    <View style={{
                        ...
                        {this.props.step.name ? <Text style={{ fontSize: 16, fontWeight: 'bold' }}>{this.props.step.name}</Text> : null}
                            {this.props.step.description ? <Text style={styles.taskDescription}>{this.props.step.description}</Text> : null}
                        {/* {this.props.step.video ?
                            <VideoPlayer
                                style={styles.backgroundVideo}
                                video={{ uri: this.apiBuilder.paths!.getVideo + this.props.step.video }}
                            /> : null} */}
