...
class DetailPage extends AbstractComponent<Props, State> {
    ...
    private interventionService: InterventionService = Bind('interventionService');
    ...
    private isTTSOn = this.storageService.getFast(StorageKeys.READ_ALL) ? true : false;
    private readDescriptionInDetail = this.storageService.getFast(StorageKeys.READ_DETAIL_DESCRIPTION) ? true : false;
    private readIngredientInDetail = this.storageService.getFast(StorageKeys.READ_DETAIL_INGREDIENT) ? true : false;
    private stepAutoOpen = this.storageService.getFast(StorageKeys.AUTO_OPEN_STEP) ? true : false;
    ...
    componentDidMount() {
        ...
        this.setState({
            ...
            //interventionJob: this.props.route?.params?.interventionData,
            workId: workId,
        ...
        this.httpClient.post(this.apiBuilder.paths!.getTask,
            {
                taskId: taskId ? taskId : '',
                isIntervention: isIntervention ? true : false,
                //interventionId: interventionId ? interventionId : '',
                    ...
                    this.setState({
                        bottomIcons: [{
                            icon: "help-circle", onClick: () => {
                                let extras = { close: this.onCloseManualPages, isIntervention: this.state.isIntervention };
                                this.eventEmitterService.emit({ type: EventTypes.Manual, data: page, extras });
                            }
                        }]
                    });
                } else {
    ...
    componentWillUnmount() {
        ...
        this.eventEmitterService.emit({ type: EventTypes.Manual, data: ManualPages.CLOSE });
        if (this.state.workId !== 0 && this.vrComponent) {
    ...
    onSnapToItem = (index: number) => {
        this.setState({
            currentPage: index,
            showDownIcon: this.state.items != null && (this.state.items?.steps.length - 1 != index),
            showUpIcon: index != 0
        }, () => {
    ...
    completeTask = () => {
        if (!this.state.isIntervention) {
            ...
            if (this.state?.content?.meta != null && this.interventionService.checkForInterventionReductionMesssage(this.state?.content?.meta)) {
                this.setState({
    ...
    renderRecipeName() {
        ...
        if (content && content.station) {
            return (<View style={{
                ...
                flexDirection: 'row',
                minHeight: this.state.recipeHeaderHeight, alignItems: 'center'
            }}>
                <Text style={{ color: getFontContrast(content.station.color), marginLeft: 15 }}>
                    {content.recipe?.name}
                </Text>
                <Text style={{ color: getFontContrast(content.station.color), flex: 1, textAlign: 'right', marginRight: 10 }}>
                    {(this.state.currentPage == 0) ? 'Overview' : 'Step ' + this.state.currentPage.toString() + ' of ' + (content.steps.length - 1).toString()}
                </Text>
            </View>);
        } else {
            return null;
    ...
    readDescriptionAndIngredients = (readDescription: boolean, readIngredient: boolean) => {
        const current = this.state.content?.steps[this.state.currentPage];
        if (!current) {
            return;
        }
        let content = readDescription && current.name ? current.name : '';
        content += " " + readDescription && current.description ? current.description : '';
    ...
    getIngredientsToRead = (page: ITaskStep) => {
        ...
        if (page.ingredients && page.ingredients.length > 0) {
            ...
            page.ingredients.forEach((ingredients) => {
                textToRead += ingredients.quantity + " " + ingredients.recipe?.outputUnits?.name + " " + ingredients?.recipe?.name + "\n";
            });
        }
        return textToRead;
    }
    ...
    closeIntervention = (reduceValue: boolean = false) => {
        ...
        return this.taskService.completeIntervention(this.state.workId, reduceValue).then(response => {
            this.props.navigation.pop();
        });
    }
    ...
    onRecognizeVoice = (action: VOICE_ACTIONS) => {
        switch (action) {
            case VOICE_ACTIONS.DONE: this.completeTask(); break;
            case VOICE_ACTIONS.STEP_BY_STEP: this.gotoFirstStep(); break;
            case VOICE_ACTIONS.READ_INGREDIENTS: this.readDescriptionAndIngredients(false, true); break;
            case VOICE_ACTIONS.DETAILS: this.readDescriptionAndIngredients(true, false); break;
            case VOICE_ACTIONS.READ_TASK: this.readDescriptionAndIngredients(true, true); break;
            case VOICE_ACTIONS.PREVIOUS: this.previousStep(); break;
            case VOICE_ACTIONS.NEXT: this.nextStep(); break;
            case VOICE_ACTIONS.SHOW_TASK: this.goBack(); break;
        }
    }
                    ...
                    {this.props.priorityTaskcount ?
                        <TouchableOpacity
                            ...
                            <Text style={{
                                ...
                                color: colors.white,
                            }}>{this.props.priorityTaskcount}</Text>
                        </TouchableOpacity>
                        : null}
                </View>
            )
    ...
    render() {
        return (
            <Appbackground
                hideBack={this.state.workId === 0 ? false : !this.state.isBackButtonVisible}
                doNaviagte={!this.state.isBackButtonVisible}
                navigation={this.props.navigation}
                bottomMenu={this.state.bottomIcons}>
                {/* <View style={{ flex: 1, flexDirection: 'column', alignSelf: 'stretch' }}/> */}
                <ConfirmationDialog show={this.state.showInterventionReductionMessage}
                ...
                <CheckingTask ref='checkingTask' pendingCallback={this.goBack} intervention={this.state.workId} />
                <VoiceRecognitionComponent
                    onRef={ref => { this.vrComponent = ref; }}
                    onRecognizeVoice={this.onRecognizeVoice} />
                {this.state.isLoading ? <ActivityIndicator style={{ flex: 1 }} size='large' color={colors.white} />
                    :
                    <View onLayout={this.onLayout.bind(this)} style={{ flex: 1, flexDirection: 'column', alignSelf: 'stretch', backgroundColor: '#fff' }}>
                        {this.renderRecipeName()}
                        <SwipeView
                            onSwipedLeft={this.showTimePicker}
                            onSwipedRight={this.completeTask}
                            disableSwipeToLeft={!(this.state.isIntervention && this.state.workId)}
                            disableSwipeToRight={!this.state.workId}
                        >
                            <Carousel
                                ref={(c: any) => { this._carousel = c; }}
                                data={this.state.content?.steps}
                                renderItem={(item: any, index: number) => this.renderPage(item.item, item.index)}
                                ...
                                initialScrollIndex={(this.state.content?.steps > 1 &&
                                    this.stepAutoOpen && this.state.workId != 0) ? 1 : 0}
                ...
                <PagerIcon icon='chevron-up' containerStyle={{ top: 0 }} onClick={this.previousStep} show={this.state.showUpIcon} />
                <PagerIcon icon='chevron-down' containerStyle={{ bottom: 0 }} onClick={this.nextStep} show={this.state.showDownIcon} />
            ...
            {(props.show) &&
                <TouchableOpacity onPress={props.onClick}>
                    <Icon style={{ alignSelf: 'center' }}
                        name={props.icon} size={30} color={colors.darkGrey} /></TouchableOpacity>
            }
        </View>
    );
}
