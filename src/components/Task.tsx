...
export default class Task extends Component<Props, State> {
  ...
  renderSecondCol(assignedStyle: any) {
    ...
    return (
      <Text style={[assignedStyle, { flex: 1 }]}>{this.props.work.output}</Text>
    );
  }
  ...
  renderTimeleft(assignedStyle: any) {
    ...
    let textForNormalTask =
      this.props.work.course && this.props.work.course.isOnCall
        ? t("order-overview.onCall")
        : timeLeft < 0
        ? 0
        : timeLeft;
    let text = this.props.isMachineTask
      ? Math.round(this.props.work.timeRemaining / 60000)
      : textForNormalTask;
    return (
      <Text
        style={[
  ...
  onTaskPress() {
    let isAssigned =
      (this.props.work.transportType & 4) == 0 &&
      this.props.work.userId &&
      this.props.work.status == WORK_STARTED;
    if (isAssigned) {
      this.showTaskDetail();
    } else {
      let isMachineTask =
  ...
  render() {
    let textColor = this.props.work.fontColor
      ? this.props.work.fontColor
      : colors.black;
    let backColor = this.props.work.backgroundColor
      ? this.props.work.backgroundColor
      : colors.white;
    let isMachineTask =
      this.props.work.task &&
      this.props.work.task.machine &&
    ...
    let assignedStyle =
      isAssigned && !isMachineTask
        ...
        : { color: textColor };
    let isAutoAssign =
      this.props.work.userObject && this.props.work.userObject.autoAssign;
    let button = [
      {
        text: "",
    ...
    let rigtButton =
      isMachineTask || isAutoAssign || !isAssigned ? undefined : button;
        ...
        >
          ...
          >
            <Text style={[styles.recipeText, { color: textColor }]}>
              {this.props.work.recipe && this.props.work.transportType != 32
                ? this.props.work.recipe.name
                : ""}
            </Text>
            <View
              ...
              style={[
                styles.container,
                {
                  shadowColor: !isMachineTask
                    ? this.props.work.shadowColor
                    : "",
                },
              ]}
            >
              {this.props.work.transportType > 0 &&
              (this.props.work.transportType & 4) === 0
                ? this.renderIcon(textColor)
                : null}
              <Text style={[styles.taskTitle, assignedStyle]}>
                {this.props.work.title
                  ? this.props.work.title
                  : this.props.work.task.name}
              </Text>
            ...
            {isAssigned && !isMachineTask ? (
              ...
              >
                {this.props.work.userObject
                  ? t("task-overview.chef-name") +
                    this.props.work.userObject.name
                  : ""}
              </Text>
            ) : null}
            <View
