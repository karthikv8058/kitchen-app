...
export default class FullWidthImage extends Component<Props, State> {
  ...
  componentDidMount() {
    let url = this.props.source
      ? this.apiBuilder.paths.imageUrl + this.props.source
      : null;
    this.setState({
      source: url,
    });
    ...
    this.httpClient
      .post(this.apiBuilder.paths.webCredentials, {})
      .then((response) => {
        this.setState({
  ...
  getImageType = (): string => {
    ...
    switch (type) {
      case ImageType.RECIPE:
        return "recipe_media";
      case ImageType.STEPS:
        return "taskstep_media";
      default:
        return "task_media";
    }
  };
  ...
  uploadImage(data) {
    ...
    NetInfo.fetch().then((state) => {
      if (state.isConnected) {
        RNFetchBlob.fetch(
          "POST",
          this.webUrl + "api/1.0/assets/upload",
          {
            Accept: "application/json",
            Authorization:
              "Bearer " +
              this.state.webAppCredentials.webCredentialData.accessToken,
          ...
          [
            ...
            {
              name: "object_uuid",
              data: this.state.webAppCredentials.webCredentialData.resturantId,
            },
          ...
          .then((resp) => {
            this.httpClient
              .post(this.apiBuilder.paths.uploadImage, {
              ...
              .then((responses) => {
                this.props.loadData();
                let url =
                  this.apiBuilder.paths.imageUrl +
                  responses.nameValuePairs.data;
                setTimeout(() => {
                  this.setState({
                    source: url,
                    isLoaderVisible: false,
        ...
        >
          ...
          >
            <View style={{ flexDirection: "row", height: 50 }}>
              ...
              >
                {t("order-overview.choose-option")}
              </Text>
              <TouchableOpacity
            ...
            <ScrollView style={{ marginBottom: 10 }}>
              <Text
                ...
                onPress={this.openImagePicker.bind(this, true)}
              >
                Camera
              </Text>
              <Text
                ...
                onPress={this.openImagePicker.bind(this, false)}
              >
                Gallery
              </Text>
            </ScrollView>
    ...
    return (
      ...
      >
        ...
        {this.state.isLoaderVisible ? (
          <ActivityIndicator
            style={{ height: 150, alignItems: "center", marginRight: 30 }}
            size="large"
            color={colors.black}
          />
        ) : (
